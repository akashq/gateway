...
function update {
	...
	vcontroli=$(</opt/gateway/versioncontrol)
	vcontrolc=$(</opt/gateway-update/gateway/versioncontrol)
	...
	then
		...
		printf "You are running the recent version\n"
		...
		then
			...
			chmod +x ./setup.sh
			./setup.sh
			printf "Your program is updated now...\n"
		else
			printf "It's always a good idea to keep your program up-to-date\n"
		fi
	fi
