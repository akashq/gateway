...
echo "0" > /opt/gateway/.cflag
echo "0" > /opt/gateway/.prevfile
...
function initialization {
	...
	while true; do
		...
		prev=$(</opt/gateway/.prevfile)
		flag=$(</opt/gateway/.flag)
		cflag=$(</opt/gateway/.cflag)
		...
		then
			...
			then
				...
				echo "$file" > /opt/gateway/.prevfile
				sleep 5
			else
				continue
			fi
		...
		then
			printf "${red}\nAll the servers except the previous ones are down\nTry another location or restart to use the previous ones\n"
			break
		elif [[ "$flag" == 1 ]]
		then
			printf "${red}\nAll the servers are currently down\nTry another location\n"
			break
		else
